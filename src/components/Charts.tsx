...
export default function Charts({ diary, selectedDays = 7, rangeDays, mood, meds = [] }: ChartsProps) {
  ...
  const [activeTooltip, setActiveTooltip] = useState<{ 
    ...
    rlsValue: number | null; 
    medsTaken?: string[];
  } | null>(null);

  const getMedicationName = (slotId: string) => {
    const medId = slotId.includes('_') ? slotId.split('_')[0] : slotId;
    const slotKey = slotId.includes('_') ? slotId.split('_')[1] : null;
    const med = meds.find(m => m.id === medId);
    if (!med) return 'Unbekanntes Med.';
    const slotLabel = slotKey ? { morning: 'Morgens', noon: 'Mittags', evening: 'Abends', night: 'Nachts' }[slotKey] : 'Bedarf';
    return `${med.name} (${slotLabel})`;
  };
        ...
        <div className="relative">
              ...
              {points.map((pt, i) => {
                ...
                return (
                  <g key={i}>
                    ...
                    {/* Pill indicator at the bottom of the grid */}
                    {pt.medsTaken.length > 0 && (
                      <rect 
                        x={x - 4} 
          ...
          {activeTooltip && (
            ...
            >
              ...
              {activeTooltip.medsTaken && activeTooltip.medsTaken.length > 0 && (
                <div className="pt-1.5 border-t border-slate-850 mt-1 text-[9px] text-slate-450 space-y-0.5">
                  <div className="font-semibold text-emerald-400">Eingenommen:</div>
                  {activeTooltip.medsTaken.map((slotId, idx) => (
                    <div key={idx} className="truncate max-w-[150px]">
                      • {getMedicationName(slotId)}
                    </div>
                  ))}
                </div>
              )}
